...
```bash
dotnet new install ermol.cleanarchitecture.template
```
...
- Обновить шаблон:
  ```bash
  dotnet new install ermol.cleanarchitecture.template --force
  ```
- Удалить шаблон:
  ```bash
  dotnet new uninstall ermol.cleanarchitecture.template
  ```
- Удалить источник:
  ```bash
...
git clone https://github.com/AlexandraErmolaeva/dotnet-clean-architecture-template
